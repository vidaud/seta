...
const DialogButton = ({ onChange, onChangeText, onChangeFile, onChangeContentVisibility }) => {
  ...
  const getTextUploadValue = (string, textSearch) => {
    ...
    if (textareaValue !== '') {
      embeddingsService
        .retrieveEmbeddings('text', { fileToUpload: textSearch, text: textareaValue })
        .then(data => setEmbeddings(data.data.embeddings.vector))
    }
  }
  ...
  const search = name => {
    if (embeddings.length > 0) {
      const lastPayload = new CorpusSearchPayload({
        ...cp,
        vector: embeddings,
        n_docs: 100,
        source: ['cordis'],
  ...
  return (
    <div className="dialog">
      ...
      />
      <Dialog
        header="Search with Document/Text"

...
const Header = () => {
  ...
  const start_seta = (
    <a href="/seta-ui/">
      <img
        alt="logo"
        src="https://raw.githubusercontent.com/AdrianaLleshi/new_deck.gl/master/images/SeTA-logocut-negative.png"
        height="40"
        className="mr-2"
      />
    </a>
  ...
  const end_seta = (
    <a href="/seta-ui/login">
      <span className="p-menuitem-icon pi pi-sign-in p-menuitem p-menuitem-link" />
    </a>
  )

  const logout = (
    <div>
      <TieredMenu model={dashboard_menu} popup ref={dashboard} id="overlay_tmenu" />
      <Button
        icon="pi pi-user"
    ...
    <div>
      ...
      <header className="seta-header">
        <Menubar
          model={items_seta}
          start={start_seta}
          end={authenticated === true ? logout : end_seta}
        />
      </header>
    </div>
  )

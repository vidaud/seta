...
export class SetaDocumentSerializer implements Serializer {
  public fromJson(json: any): SetaDocument {
    ...
    doc.date =
      Array.isArray(json.date) && json.date.length > 0
        ? json.date[0]
        : null || json.date?.constructor === String
        ? json.date
        : null
    ...
    if (json.concordance) {
      doc.concordance = [...json.concordance].map(data => {
        return new ConcordancePermutation({
          contextLx: data[0],
          keyword: data[1],
          contextRx: data[2]
        })
    ...
    if (json.embeddings) {
      doc.embeddings = new EmbeddingsModel({
        vector: [...json.embeddings.vector],
        version: json.embeddings.version
      })
    }

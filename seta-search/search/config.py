...
class Config:
    ...
    def _init_config_section(config_section: configparser.SectionProxy):
        ...
        # flask
        Config.SECRET_KEY = config_section.get(
            "SECRET_KEY", fallback="no-need-for-secret"
        )
        Config.PROPAGATE_EXCEPTIONS = config_section.getboolean(
            "PROPAGATE_EXCEPTIONS", fallback=True
        )
        Config.TESTING = config_section.getboolean("TESTING", fallback=False)
        Config.MAX_CONTENT_LENGTH = config_section.getint(
            "MAX_CONTENT_LENGTH", fallback=52_428_800
        )
        ...
        Config.JWT_COOKIE_CSRF_PROTECT = config_section.getboolean(
            "JWT_COOKIE_CSRF_PROTECT", fallback=True
        )
        Config.JWT_COOKIE_SECURE = config_section.getboolean(
            "JWT_COOKIE_SECURE", fallback=False
        )

        token_location = config_section.get(
            "JWT_TOKEN_LOCATION", fallback="headers,cookies"
        )
        ...
        # restx
        Config.RESTX_MASK_SWAGGER = config_section.getboolean(
            "RESTX_MASK_SWAGGER", fallback=False
        )
        Config.SWAGGER_UI_JSONEDITOR = config_section.getboolean(
            "SWAGGER_UI_JSONEDITOR", fallback=False
        )

        # custom
        Config.DEFAULT_SUGGESTION = config_section.getint(
            "DEFAULT_SUGGESTION", fallback=6
        )
        Config.DEFAULT_TERM_NUMBER = config_section.getint(
            "DEFAULT_TERM_NUMBER", fallback=20
        )
        Config.DEFAULT_DOCS_NUMBER = config_section.getint(
            "DEFAULT_DOCS_NUMBER", fallback=10
        )
        Config.DEFAULT_FROM_DOC_NUMBER = config_section.getint(
            "DEFAULT_FROM_DOC_NUMBER", fallback=0
        )
        Config.PAGINATION_DOC_LIMIT = config_section.getint(
            "PAGINATION_DOC_LIMIT", fallback=0
        )
        Config.KNN_SEARCH_K = config_section.getint("KNN_SEARCH_K", fallback=5000)
        Config.KNN_SEARCH_NUM_CANDIDATES = config_section.getint(
            "KNN_SEARCH_NUM_CANDIDATES", fallback=5000
        )
        Config.SIMILARITY_THRESHOLD = config_section.getfloat(
            "SIMILARITY_THRESHOLD", fallback=0
        )
        Config.AGG_MISSING_NAME = config_section.get(
            "AGG_MISSING_NAME", fallback="NO_CLASS"
        )

        search_types = config_section.get(
            "SEARCH_TYPES", fallback="DOCUMENT_SEARCH,CHUNK_SEARCH,ALL_CHUNKS_SEARCH"
        )
        Config.SEARCH_TYPES = search_types.split(sep=",")

        Config.DEFAULT_ENRICHMENT_TYPE = config_section.get(
            "DEFAULT_ENRICHMENT_TYPE", fallback="similar"
        )
        Config.EXPORT_DOCUMENT_LIMIT = config_section.getint(
            "EXPORT_DOCUMENT_LIMIT", fallback=5000
        )
        ...
        Config.NLP_API_ROOT_URL = config_section.get("NLP_API_ROOT_URL")
        ...
        Config.SCHEDULER_LOG_NAME = config_section.get(
            "SCHEDULER_LOG_NAME", "sched.log"
        )
        Config.LOG_MAX_BYTES = config_section.getint(

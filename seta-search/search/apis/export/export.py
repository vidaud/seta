...
export_req = {
    "ids": fields.List(fields.Nested(id_model)),
    ...
    def post(self):
        try:
            ...
            if export_format not in export_formats:
                raise ApiLogicError(
                    "Invalid Accept_mimetypes, it must be application/json or text/csv"
                )
...
class ExportResource(Resource):
    @export_api.doc(
        description="Available document fields and descriptions are returned.",
        responses={200: "Success", 404: "Not Found Error"},
        security="apikey",
    )
    ...
    def get(self):
        try:
            ...
            # get catalogue from
            result = requests.get(
                url=url, headers=request.headers, cookies=request.cookies, timeout=10
            )

from search.infrastructure.utils.crc import get_crc_from_es
...
def get_word_suggestions(current_app, chars, n_suggestions=6):
    ...
    body = {"query": {"bool": {"must": [{"prefix": {"phrase.keyword": {"value": chars}}},
                                        {"match": {"crc.keyword": current_crc}}]}}}
    
    resp = current_app.es.search(index=current_app.config["INDEX_SUGGESTION"], body=body, size=n_suggestions)

...
class Config:
    ...
    def __init__(self, section_name: str) -> None:
        ...
        if not sections:
            message = (
                "No configuration section found in the config files"
                f"('{Config.CONFIG_LOGS_FILE}','{Config.CONFIG_APP_FILE}')"
            )
    ...
    def _init_config_section(config_section: configparser.SectionProxy):
        ...
        Config.DISABLE_SWAGGER_DOCUMENTATION = config_section.getboolean(
            "DISABLE_SWAGGER_DOCUMENTATION", fallback=True
        )
        Config.AUTO_APPROVE_CHANGE_REQUEST = config_section.getboolean(
            "AUTO_APPROVE_CHANGE_REQUEST", fallback=False
        )

        identity_providers = config_section.get(
            "SETA_IDENTITY_PROVIDERS", fallback="ECAS"
        )
        ...
        Config.JWT_ACCESS_TOKEN_EXPIRES = config_section.getint(
            "JWT_ACCESS_TOKEN_EXPIRES", fallback=900
        )
        Config.JWT_REFRESH_TOKEN_EXPIRES = config_section.getint(
            "JWT_REFRESH_TOKEN_EXPIRES", fallback=86400
        )

        Config.SCHEDULER_API_ENABLED = config_section.getboolean(
            "SCHEDULER_API_ENABLED", fallback=False
        )

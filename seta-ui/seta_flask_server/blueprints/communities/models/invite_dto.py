...
new_invite_parser.add_argument(
    "email",
    location="form",
    required=True,
    nullable=False,
...
new_invite_parser.add_argument(
    "message", location="form", required=True, nullable=False, help="Invite message"
)

update_invite_parser = RequestParser(bundle_errors=True)
update_invite_parser.add_argument(
    "status",
    location="form",
    required=True,
    nullable=False,
...
invite_model = Model(
    "Invite",
    {
        ...
        "invited_user_info": fields.Nested(
            model=user_info_model, description="Invited user info", skip_none=True
        ),
        "message": fields.String(description="Sent invite message"),
        "status": fields.String(
            description="The invite status", enum=InviteStatusConstants.List
        ),
        "expire_date": fields.DateTime(description="Community join date"),
        "initiated_by": fields.String(description="Sender identifier"),
        "initiated_by_info": fields.Nested(
            model=user_info_model, description="Sender info", skip_none=True
        ),
        "initiated_date": fields.DateTime(description="Send date"),
    },
)

ns_models = {user_info_model.name: user_info_model, invite_model.name: invite_model}

...
def parse_args_new_resource(
    creator_id: str, community_id: str, resource_dict: dict
) -> (ResourceModel, list[EntityScope]):
    """Parse resource api json."""
    ...
    scopes = [
        EntityScope(
            user_id=creator_id, id=resource_id, scope=ResourceScopeConstants.Edit
        ).to_resource_json(),
        EntityScope(
            user_id=creator_id, id=resource_id, scope=ResourceScopeConstants.DataAdd
        ).to_resource_json(),
        EntityScope(
            user_id=creator_id, id=resource_id, scope=ResourceScopeConstants.DataDelete
        ).to_resource_json(),
    ]

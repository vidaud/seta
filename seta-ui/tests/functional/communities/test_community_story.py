...
from tests.infrastructure.helpers.community import (
    create_community,
    get_community,
    update_community,
    delete_community,
)
from tests.infrastructure.helpers.resource import (
    create_resource,
    get_resource,
    update_resource,
    delete_resource,
...
from tests.infrastructure.helpers.community_membership import (
    create_membership_request,
    get_membership_requests,
    update_membership_request,
    delete_membership,
)
from tests.infrastructure.helpers.community_permission import (
    replace_user_permissions,
    get_user_permissions,
)
from tests.infrastructure.helpers.resource_permission import (
    replace_user_permissions_for_resource,
...
def test_create_community(
    ...
    response = get_community(
        client=client, access_token=access_token, community_id="blue"
    )
...
def test_create_resource(
    ...
    response = get_resource(
        client=client, access_token=access_token, resource_id=resource["resource_id"]
    )
    assert response.status_code == HTTPStatus.OK
...
def test_fail_create_resource_forbidden(
    ...
    response = create_resource(
        ...
        resource_id="sea",
        title="Sea",
        abstract="Sea resource for test",
    )
    assert response.status_code == HTTPStatus.FORBIDDEN
...
@pytest.mark.parametrize(
    "user_id, community_id, expect",
    [
        ("seta_admin", "blue", HTTPStatus.OK),
...
@pytest.mark.parametrize(
    "user_id, community_id, request_id",
    [
        pytest.param(
            "seta_community_manager",
            "blue",
            "seta_community_manager",
        ...
        pytest.param(
            "seta_admin", "blue", "seta_community_manager", marks=pytest.mark.xfail
        ),
    ],
)
...
@pytest.mark.parametrize(
    "user_id, community_id, manager_id",
    [("seta_admin", "blue", "seta_community_manager")],
)
def test_add_community_manager(
    ...
    response = replace_user_permissions(
        client=client,
        access_token=access_token,
        community_id=community_id,
        user_id=manager_id,
...
def test_get_community_join_requests_again(
    ...
    test_get_community_join_requests(
        client=client,
...
@pytest.mark.parametrize(
    "user_id, community_id, member_id",
    [("seta_community_manager", "blue", "seta_admin")],
)
...
def test_fail_create_resource_forbidden_no_membership(
    ...
    response = create_resource(
        ...
        resource_id="lake",
        title="Lake",
        abstract="Lake resource for test",
    )
    assert response.status_code == HTTPStatus.FORBIDDEN
...
@pytest.mark.parametrize(
    ...
    [
        ("seta_community_manager", "ocean", HTTPStatus.FORBIDDEN),
...
def test_add_resource_permission(
    ...
    response = get_user_permissions_for_resource(
        client=client,
        access_token=access_token,
        resource_id=resource_id,
        user_id=user_id,

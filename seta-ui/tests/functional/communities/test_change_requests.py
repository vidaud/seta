...
def test_community_approve_change_request(client: FlaskClient, authentication_url:str, user_id: str):  
    ...
    response = update_community_change_request(client=client, access_token=access_token, 
                community_id=cr["community_id"], request_id=cr["request_id"], status=RequestStatusConstants.Approved)
    assert response.status_code == HTTPStatus.OK

    response = get_community(client=client, access_token=access_token, id=cr["community_id"])
    assert response.status_code == HTTPStatus.OK
    assert CommunityRequestFieldConstants.Membership in response.json
    assert response.json[CommunityRequestFieldConstants.Membership] == CommunityMembershipConstants.Opened

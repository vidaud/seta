...
def test_get_all_roles(client: FlaskClient, authentication_url:str, user_id: str):
    ...
    assert len(response.json["application"]) > 0

    assert "community" in response.json
    assert len(response.json["community"]) > 0

@pytest.mark.parametrize("user_id, category", [("seta_admin", "application"), ("seta_admin", "community")])
def test_get_roles_by_category(client: FlaskClient, authentication_url:str, user_id: str, category):
    response = login_user(auth_url=authentication_url, user_id=user_id)    
    assert response.status_code == HTTPStatus.OK
    response_json = response.json()
    assert "access_token" in response_json
    ...
    response = get_roles_by_category(client=client, access_token=access_token, category=category)
    assert response.status_code == HTTPStatus.OK

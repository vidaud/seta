...
def get_user_permissions_for_resource(client: FlaskClient, access_token:str, resource_id: str, user_id: str):
    url = f"{API_V1}/permissions/resource/{resource_id}/user/{user_id}"

    return client.get(url, headers=auth_headers(access_token))

def replace_user_permissions_for_resource(client: FlaskClient, access_token:str, resource_id: str, user_id: str, scopes:list[str]):
    url = f"{API_V1}/permissions/resource/{resource_id}/user/{user_id}"

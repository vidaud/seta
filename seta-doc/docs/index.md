...
**SeTA** (“Semantic Text Analyser”) is a tool that offers communities, a search functionality in large document collections.

## Communities

SeTA communities is a shared place, where users can have the possibility to interact with others about specific areas of interest. The users can create different Communities according to their specific matter of interest, and share, and debate with other users with similar interests.        
...
SeTA also provides REST APIs created with Swagger UI[^2]. Through this it is possible to have a visual representation of the API together with its documentation allowing users to try out the API calls directly from the browser.          
